...
async def generate_review_report(
    ...
    for req in evidence_data.get("evidence", []):
        ...
        # Extract page citations with section numbers
        page_citations = []
        for snippet in req.get("evidence_snippets", []):
            doc_name = snippet.get("document_name", "Unknown")
            page = snippet.get("page")
            section = snippet.get("section")
            citation = _format_citation(doc_name, section, page)
            if citation:
                page_citations.append(citation)
...
def format_checklist_report(
    ...
    # Get project_id for document references
    project_id = project.get("project_id")

    for req in report.requirements:
        evidence_item = evidence_lookup.get(req.requirement_id, {})
        row = _format_checklist_row(req, evidence_item, project_id)
...
def _format_citation(doc_name: str, section: str | None, page: int | None) -> str | None:
    """Format a document citation with section and page.

    Returns citation in format: "Doc.pdf (Section X.Y, p.Z)" or variations
...
def _format_submitted_material(
    ...
    **Supplementary:** [additional sources]
    ...
    Evidence text is returned separately for the Comments column.
    """
    if not snippets:
    ...
    # Extract value: prioritize extracted_value or structured_fields over text truncation
    value_text = _extract_value(primary)

    # Format document name with project ID prefix if available
    if project_id:
        doc_ref = f"[{project_id}] {doc_name}"
    else:
        doc_ref = doc_name
    ...
    supplementary = snippets[1:4] if len(snippets) > 1 else []
    if supplementary:
        supp_citations = []
        for supp in supplementary:
            supp_doc = supp.get("document_name", "Unknown")
            if project_id:
                supp_doc = f"[{project_id}] {supp_doc}"
            supp_section = supp.get("section")
            supp_page = supp.get("page")
            supp_citation = _format_citation(supp_doc, supp_section, supp_page) or supp_doc
            supp_citations.append(supp_citation)
        result_parts.append(f"**Supplementary:** {'; '.join(supp_citations)}")

    submitted_material = "<br>".join(result_parts)

    # Evidence text for Comments column (with sentence-boundary truncation)
    if text:
        clean_text = " ".join(text.split())  # Normalize whitespace
        evidence_text = _truncate_at_sentence(clean_text, max_length=1000)
    else:
        evidence_text = ""

    return submitted_material, evidence_text


def _extract_value(snippet: dict[str, Any]) -> str:
    """Extract a concise value from a snippet.

    Priority:
    1. structured_fields.value or structured_fields.extracted_value
    2. extracted_value field directly on snippet
    3. First key-value from structured_fields that looks like a value
    4. Fall back to first sentence of text (truncated)
    """
    # Check structured_fields first
    fields = snippet.get("structured_fields", {})
    if fields:
        # Look for explicit value fields
        for key in ["value", "extracted_value", "answer"]:
        ...
        # Use first non-empty string value if available
        for key, val in fields.items():
            if isinstance(val, str) and val.strip():
                return val

    # Check direct extracted_value on snippet
    extracted = snippet.get("extracted_value")
    if extracted:
    ...
    if text:
        ...
        if sentence_end > 0 and sentence_end < 200:
            ...
            return _truncate_at_sentence(clean, max_length=200)

    return "_No value extracted_"


def _truncate_at_sentence(text: str, max_length: int = 1000) -> str:
    """Truncate text at sentence boundary, never mid-sentence.

    Args:
        text: Text to truncate
        max_length: Maximum length (default 1000, configurable)
    ...
    truncation_point = -1
    for ending in [". ", ".\n", ".\t"]:
        pos = text[:max_length].rfind(ending)
        if pos > truncation_point:
            truncation_point = pos + 1  # Include the period

    # If we found a good sentence boundary in the latter half of allowed text
    if truncation_point > max_length * 0.5:
        return text[:truncation_point]

    # Fall back: find last space to avoid mid-word truncation
...
def _format_checklist_row(
    ...
    submitted_material, evidence_text = _format_submitted_material(snippets, project_id)
    ...
    # Add evidence text to Comments column (Issue 4)
    if evidence_text:
    ...
    comments = "<br>".join(comments_parts) if comments_parts else ""
...
def _set_cell_evidence_formatted(
    ...
    # Extract concise value (Issue 2)
    value = _extract_value(primary)

    # Format document reference with project ID (Issue 6)
    if project_id:
        doc_ref = f"[{project_id}] {doc_name}"
    else:
        doc_ref = doc_name
    ...
    # Primary Documentation: label (bold) + citation with section
    ...
    # Add supplementary evidence (Issue 3)
    supplementary = snippets[1:4] if len(snippets) > 1 else []
    if supplementary:
        run = para.add_run("Supplementary: ")
        run.font.bold = True
        run.font.color.rgb = blue
        supp_citations = []
        for supp in supplementary:
            ...
            supp_citation = _format_citation(supp_doc, supp_section, supp_page) or supp_doc
            supp_citations.append(supp_citation)
        run = para.add_run("; ".join(supp_citations))
        run.font.color.rgb = blue

    # Return evidence text for Comments column (Issue 4, 5)
    if text:
        clean = " ".join(text.split())
        return _truncate_at_sentence(clean, max_length=1000)
    return ""
...
def format_docx_report(
    ...
    if len(doc.tables) >= 2:
        ...
        # Get project_id for document references (Issue 6)
        project_id = project.get("project_id")
        ...
        for row_idx, req in enumerate(report.requirements, start=2):  # Skip row 0 (merged header) and row 1 (column headers)
            ...
            # Column 3: Submitted Material (Value + Primary/Supplementary docs)
            evidence_text = _set_cell_evidence_formatted(row.cells[3], evidence_item, project_id)
            ...
            # Column 5: Comments (Confidence + Evidence text + notes) (Issue 4)
            comments = _get_docx_comments_with_evidence(req, evidence_text)
...
def _get_docx_comments_with_evidence(req: RequirementFinding, evidence_text: str) -> str:
    """Generate comments text for DOCX including evidence text (Issue 4).
