...
def check_date_alignment(
    fields_by_doc: dict[str, dict[str, Any]], max_delta_days: int = 120
...
def check_name_consistency(
    fields_by_doc: dict[str, dict[str, Any]], fuzzy_threshold: float = 0.8
) -> list[CrossDocumentCheck]:
        ...
        if len(unique_names) == 1:
            ...
            for i in range(len(values)):
                for j in range(i + 1, len(values)):
                    sim = SequenceMatcher(None, values[i].lower(), values[j].lower()).ratio()
...
def check_numeric_consistency(
    ...
    numeric_fields = ["area_hectares", "buffer_pool_percentage", "crediting_period_years", "permanence_period_years"]
...
def run_cross_document_checks(evidence_data: dict) -> CrossDocumentValidationResult:
    ...
    logger.info(
        f"Cross-document checks complete: {result.passed} passed, {result.warnings} warnings, {result.failed} failed"
    )
