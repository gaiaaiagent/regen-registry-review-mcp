"""Slow regression test — asserts full-pipeline agreement ≥ v2.2.0 baseline.
...
def test_v2_2_0_baseline(fixture: str, telus_available: bool, tmp_path: Path):
    ...
    baseline = BASELINES_DIR / f"{fixture}-v2.2.0.json"
    ...
    art_dir = output_root / "phase-c" / "artifacts"
    ...
    diff_jsons = sorted(
        (output_root / "phase-c" / "diffs").glob(f"{fixture}-*-diff.json"),
        reverse=True,
    )
